...
<div class="tablelist margin">
    ...
    <thead>
     ...
     <tr>
     <td><input name="stuId" type="checkbox" value="${s.stuId }" /></td> 
     <td align="center">${s.studentName}</td>
     <td align="center">${s.subName} </td>
     <td align="center">${s.teaName }</td>
     <td align="center">${s.starTime}</td>
        <td>
        <a href="javascript:deleteStu()" class="tablelink">删除</a>
        </td>
      </tr>
      </c:forEach>
    </tbody>
     ...
     <!-----------分页----------->
            	<div id="page" class="paginationBox clear margin mt20" >
                                      ...
                                      <c:forEach  var="item" varStatus="status" begin="1" end="${pager.maxPageNum-1}">
 										<span ><a yangshi="1" href="javascript:doSub(${status.index })" title="${status.index }" >${status.index }</a></span>
 										</c:forEach>
