...
<script>
	...
	function doSub(pageNum) {
		//同一个URL不会重新加载
		$("#listTable").load("homeworkListTable?o="+new Date().getTime(), {
 ...
 <div class="search">
    ...
    <script type="text/javascript">
            function checkAll(checkbox){
            	...
            	function deleteRes(){
            		...
            		for(var i = 0;i<cks.length;i++){
            			if($(cks[i]).attr("type")=='checkbox' && $(cks[i]).attr("name")=='workId'){
            				//alert($(cks[i]).val());
            				params+="&workId="+$(cks[i]).val();
            			}
            		};
            		
            		var url = "/Team3_ZXJY/deleteWork?"+params;
            		if(confirm("您确定删除吗?")){
            			location.href = url;
            		}
            		//跳转
     ...
     <div class="applybutton mt clear">
      <a href="/Team3_ZXJY/jspf/addHomework.jsp">添 加</a>
      <a href="javascript:deleteRes()">删 除</a>
     </div>
  <!-- 列表 -->
