...
function doSub(pageNum){
		//alert(pageNum);
		//同一个URL不会重新加载
		$("#listTable").load("/Team3_ZXJY/questionList?o="+new Date().getTime(), {
			title : $("#title").val(),
			pageNum:pageNum
		},function(){
			f1();
 ...
 <div class="search">
      ...
      <table border="0" cellpadding="0" cellspacing="0" class="table01">
        <tr>
          ...
          <td width="80" onclick="doSub()" align="right"><input class="button" type="submit" name="button" id="button" value="查&nbsp;询" /></td>
        </tr>
      </table>
    </div>
    <script type="text/javascript">
    function checkAll(checkbox){
    	...
    	function deleteQue(){
    		...
    		for(var i = 0;i<cks.length;i++){
    			if($(cks[i]).attr("type")=='checkbox' && $(cks[i]).attr("name")=='queId'){
    				//alert($(cks[i]).val());
    				params+="&queId="+$(cks[i]).val();
    			}
    		};
    		
    		var url = "/Team3_ZXJY/questionDelete?"+params;
    		if(confirm("您确定删除吗?")){
    			location.href=url;
    		}
    		//跳转
     ...
     <div class="applybutton mt clear">
      <a href="/Team3_ZXJY/Stu_choseTeacher">添 加</a>
